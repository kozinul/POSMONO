...
> **Dokumentasi lengkap: `docs/QRIS_GATEWAY_PLAN.md`** (kontrak API, keputusan desain, file inventory). Status: **SELESAI & TER-COMMIT** — backend + frontend PaymentModal + Settings UI + tests + simulator + docs (`API_REFERENCE.md` §Payments/QRIS, `POS_CURRENT_FEATURES.md` §22). `tools/qris-simulator/data/` di-gitignore.
...
- **Config tenant baru**: `qrisGatewayEnabled/BaseUrl/ApiKey/MerchantId` di `TenantSchema` + shared `updateTenantConfigSchema` (refine wajib lengkap saat enabled); default baseUrl `http://localhost:5000`; frontend `TenantConfig` (useTenant.ts) +4 field opsional
...
- **Simulator**: `tools/qris-simulator/server.mjs` — mock gateway port 5000, generate EMVCo TLV QR dinamis asli + CRC16, aksi create-invoice/check-status/pay/void/list, expiry 15m, persist `data/invoices.json`, env override (`QRIS_API_KEY=12345678`, `QRIS_MERCHANT_ID=123456`)
...
- **Sisa kerja**: tidak ada — fitur lengkap & ter-commit (E2E manual dengan simulator belum dijalankan: docker tidak tersedia di env ini)
